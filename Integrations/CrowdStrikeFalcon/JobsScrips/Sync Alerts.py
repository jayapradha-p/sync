...
class SyncAlerts(BaseSyncJob[CrowdStrikeManager]):
    ...
    def modified_synced_case_ids_by_product(
        ...
        alerts_data = self.manager.get_alerts_details(alert_ids)
    ...
    def is_alert_and_product_closed(
        ...
        alert = next(
            (
                ...
                if utils.get_alert_id_from_alert(self.soar_job, alert)
                == product.composite_id
            ),
            None,
        )
    ...
    def _extract_product_ids_from_case(self, job_case: JobCase) -> list[str]:
        ...
        for alert in job_case.case_detail.alerts:
            ticket_id = utils.get_alert_id_from_alert(self.soar_job, alert)
            if ticket_id:
                alert_ids.append(ticket_id)
    ...
    def _populate_alert_metadata(
        ...
        for alert in job_case.case_detail.alerts:
            for product_detail in product_details:
                if product_detail.composite_id == utils.get_alert_id_from_alert(
                    self.soar_job, alert
                ):
                    verdict = None
                    if (
    ...
    def _sync_case_status_request_to_product(
        ...
        self._apply_case_closure_to_product(job_case, req, verdict)
    ...
    def _build_product_comments_for_alert(
        ...
        for product_comment in alert.incident.comments:
            ...
            comment_hash = job_case._generate_string_hash(comment_with_product_prefix)

            if (
                job_case._is_valid_product_comment(comment_text, case_comment_prefix)
                ...
                if constants.SECOPS_CASE_TAG in alert_identifier:
                    alert_identifier_prefix = comment.split(":")[0]
                    comment = comment.replace(f"{alert_identifier_prefix}:", "", 1)
    ...
    def sync_case_comments_to_product(
        ...
        for alert in self._get_unique_alerts(job_case):
            for comment in comments:
                self._push_comment_to_alert(alert, comment)

    def _get_unique_alerts(self, job_case: JobCase) -> list[AlertCard]:
        """Returns open alerts in the case for comment sync.

        Args:
            job_case (JobCase): The SecOps case to inspect.

        Returns:
            list[AlertCard]: A list of open AlertCards with incidents.
        """
        unique_alerts = []
        ...
        for alert in job_case.case_detail.alerts:
            if not self._is_valid_alert(alert):
                continue
            alert_closed_status = constants.ALERT_CLOSED_STATUS.lower()
            if alert.incident.raw_data.get("status") == alert_closed_status:
                continue
    ...
    def _push_comment_to_alert(self, alert: AlertCard, text: str) -> None:
        """Pushes a comment to the CrowdStrike alert.

        Args:
            alert (AlertCard): The alert object containing incident details.
            text (str): The comment text to push to the CrowdStrike alert.
        """
        self.manager.add_comment_to_alert(alert.incident.composite_id, text)
        self.logger.info(
            f"Successfully synced comments from SecOps to CrowdStrike alert "
            f"{alert.incident.composite_id}."
        )
    ...
    def remove_synced_data_from_db(
        ...
        """
        for alert in job_case.case_detail.alerts:
            alert_id = utils.get_alert_id_from_alert(self.soar_job, alert)
