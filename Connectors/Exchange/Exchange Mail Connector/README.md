...
|----|-----------|------------|-----|
|Server Ip|x.x.x.x|True|192.168.29.234|
|Domain|Specify the domain value to use for authentication.|True|domain.com|
|Script Timeout (Seconds)|The timeout limit (in seconds) for the python process running current script|True|60|
|Username|Specify Username to authenticate with on mail server. Username can be provided without the domain part or in either UPN (user@fully_qualified_DNS_domain_name) or Down-Level Logon Name (domain\username) format.|False|hello|
|Password|Password|True|*****|
|Mail Address|Mail address to pull emails from. e.g. user@domain.com|True|jaya|
